...
Astro.eventManager.on(
  'initDefinition', function onInitDefinitionFields(schemaDefinition) {
    ...
    _.each(fieldsDefinitions, function(fieldDefinition, fieldPattern) {
      ...
      schema.fields[fieldPattern] = field;

      // Create an object tree with nested fields names.
      Astro.utils.fields.traverseObject({
        object: schema.fieldsNames,
        pattern: fieldPattern,
        options: {
          create: true
        },
        callback: function(nestedObject, segment, segments) {
          nestedObject[segment] = nestedObject[segment] || {};
        }
      });
    });
  }
